...
export async function createEmployee({ name, position, department, salary, employmentHistory, contact }) {
  const [result] = await pool.query(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `, [name, position, department, salary, employmentHistory || null, contact]);
...
export async function updateEmployee(employeeId, { name, position, department, salary, employmentHistory, contact }) {
  const [result] = await pool.query(`
    ...
    SET name = ?,
        ...
        contact = ?
    WHERE employee_id = ?
  `, [name, position, department, salary, employmentHistory || null, contact, employeeId]);
...
export async function getEmployeePayroll(employeeId) {
  const [rows] = await pool.query(`
    ...
    FROM payroll
    WHERE employee_id = ?
    ORDER BY COALESCE(pay_period_end, created_at) DESC, payroll_id DESC
  `, [employeeId]);
...
export async function getEmployeeLeave(employeeId) {
  const [rows] = await pool.query(`
    ...
    FROM leave_requests
    WHERE employee_id = ?
    ORDER BY date DESC, request_id DESC
  `, [employeeId]);
...
export async function getEmployeeSummary(employeeId) {
  ...
  const [[leave]] = await pool.query(`
    SELECT
      ...
      COALESCE(SUM(status = 'Denied'), 0) AS deniedRequests
    FROM leave_requests
    WHERE employee_id = ?
  `, [employeeId]);
