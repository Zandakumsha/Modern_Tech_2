  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT event_id AS id, DATE_FORMAT(event_date, '%Y-%m-%d') AS eventDate,
        ...
        ${user ? "WHERE user_id = ? OR user_id IS NULL" : ""}
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO calendar_events (user_id, hr_username, event_date, title, event_time, category, description)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [user?.user_id ?? null, user?.username ?? null, date, cleanTitle, eventTime, category, String(description || "").trim()]);

    const [rows] = await pool.query(
      `SELECT event_id AS id, DATE_FORMAT(event_date, '%Y-%m-%d') AS eventDate,
              title, TIME_FORMAT(event_time, '%H:%i') AS time,
              category, description, hr_username
         FROM calendar_events WHERE event_id = ?`, [result.insertId]);
...
export async function deleteEvent(req, res) {
  try {
    ...
    const [result] = await pool.query(
      user ? "DELETE FROM calendar_events WHERE event_id = ? AND (user_id = ? OR user_id IS NULL)" : "DELETE FROM calendar_events WHERE event_id = ?",
      user ? [eventId, user.user_id] : [eventId]);
