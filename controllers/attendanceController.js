...
const leaveSelect = `
  SELECT lr.request_id AS requestId,
...
export async function getEmployeeAttendance(req, res) {
  ...
  try {
    ...
    const [leaveRequests] = await pool.query(`${leaveSelect} WHERE lr.employee_id = ? ORDER BY lr.date DESC`, [employeeId]);
...
export async function getLeaveRequests(req, res) {
  try {
    const [rows] = await pool.query(`${leaveSelect} ORDER BY lr.date DESC`);
    res.json(rows);
  } catch (error) {
