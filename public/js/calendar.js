...
if (calendar && miniCalendar && monthTitle && headerMonth && eventsList && selectedDateTitle && eventForm) {
  ...
  function getLoggedInUsername() {
    const candidates = [
      localStorage.getItem("username"), localStorage.getItem("hr_username"),
      localStorage.getItem("user"), localStorage.getItem("currentUser"),
      sessionStorage.getItem("username"), sessionStorage.getItem("hr_username"),
  ...
  async function loadEvents() {
    ...
    try {
      ...
      const apiEvents = Array.isArray(data) ? data : Array.isArray(data?.events) ? data.events : Array.isArray(data?.data) ? data.data : [];
  ...
  async function createEvent(eventData) {
    ...
    if (!hrUsername) throw new Error("The logged-in HR username could not be found. Please log in again.");
  ...
  async function deleteEvent(eventId) {
    if (eventId === undefined || eventId === null || eventId === "") throw new Error("This event does not have a valid database ID.");
  ...
  eventForm.addEventListener("submit", async e => {
    ...
    try {
      await createEvent({ eventDate: selectedDateKey, title, time, category, description });
      eventForm.reset();
