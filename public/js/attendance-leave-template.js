(() => {
  ...
  const formatDate = (value) => value ? new Date(`${value}T00:00:00Z`).toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "numeric", timeZone: "UTC" }) : "";
  ...
  function requestDetails(request) {
    ...
    const selectedReasons = reasonOptions.map(([key, label]) => {
      const checked = reasonChecked(request, label);
      const detail = checked ? (label === reason ? "" : reason) : "";
      return `<label class="leave-reason-option"><input type="checkbox" ${checked ? "checked" : ""} disabled><span>${escapeHtml(label)}</span><input class="leave-reason-line" value="${escapeHtml(detail)}" readonly></label>`;
    }).join("");

    return `
      <div class="leave-view-modal" role="dialog" aria-modal="true" aria-labelledby="attendanceLeaveViewTitle">
        <div class="leave-view-dialog">
          <div class="leave-form-card">
            ...
            <div class="leave-form-body leave-form-readonly">
              <div class="leave-emp-grid">
                <label>Employee Name</label><input class="leave-line" value="${escapeHtml(name)}" readonly>
                <label>Date</label><input class="leave-line" value="${escapeHtml(formatDate(requestDate || request.date))}" readonly>
                <label>Department</label><input class="leave-line" value="${escapeHtml(department)}" readonly>
                <label>Supervisor Name</label><input class="leave-line" value="${escapeHtml(supervisor)}" readonly>
              </div>
              <div class="leave-section-title">REASON FOR LEAVE</div>
              <div class="leave-reason-grid">${selectedReasons}</div>
              <div class="leave-section-title">LEAVE REQUESTED</div>
              <div class="leave-request-grid">
                <label>From</label><input class="leave-line" type="date" value="${escapeHtml(startDate)}" readonly>
                <label>Time</label><input class="leave-line" type="time" value="${escapeHtml(request.startTime || "")}" readonly>
                <label>a.m./p.m.</label><span></span>
                <label>Total Number of Hours Requested</label><input class="leave-line" type="number" value="${escapeHtml(request.hours || "")}" readonly>
                <label>To</label><input class="leave-line" type="date" value="${escapeHtml(endDate)}" readonly>
                <label>Time</label><input class="leave-line" type="time" value="${escapeHtml(request.endTime || "")}" readonly>
                <label>a.m./p.m.</label><span></span>
                <label>Total Number of Days Requested</label><input class="leave-line" value="${escapeHtml(days)}" readonly>
                <label>Other</label><input class="leave-line wide" value="${escapeHtml(request.reasonNotes || "")}" readonly>
                <label class="wide">Additional Notes</label><input class="leave-line wide" value="${escapeHtml(request.notes || "")}" readonly>
              </div>
              ...
              <div class="leave-supervisor-box">SUPERVISOR USE ONLY</div>
              <label class="leave-field-label">Comments</label><textarea class="leave-comments" readonly>${escapeHtml(request.comments || "")}</textarea>
              <div class="leave-supervisor-fields"><label>Approved By:</label><input value="${escapeHtml(request.approvedBy || request.managerName || "")}" readonly><label>Supervisor Signature:</label><input value="${escapeHtml(request.managerSignature || "")}" readonly><label>Date:</label><input type="date" value="${escapeHtml(request.managerDate || "")}" readonly></div>
              <div class="leave-status-line ${escapeHtml(status)}">Status: ${escapeHtml(request.status || "Pending")}</div>
            </div>
            <div class="leave-form-actions"><button class="leave-form-btn" type="button">Exit</button></div>
          </div>
        </div>
      </div>`;
  }
  ...
  function render() {
    ...
    panel.innerHTML = `
      ...
      <div class="leave-template-card"><div class="leave-pending-list">${pending.length ? pending.map((r) => `
        <div class="leave-pending-row"><div class="leave-person">${escapeHtml(r.name || employeeName(r.employeeId))}</div><div class="leave-meta">${escapeHtml(r.reason || "Leave request")} · ${escapeHtml(formatDate(r.date))}</div><div class="leave-actions"><button class="leave-action approve" data-template-leave-action="Approved" data-request-id="${r.requestId}">Approve</button><button class="leave-action deny" data-template-leave-action="Denied" data-request-id="${r.requestId}">Deny</button><button class="leave-action view" data-view-leave-request="${r.requestId}">View Leave Request</button></div></div>`).join("") : `<div class="leave-empty">No pending requests</div>`}</div></div>
    ...
    panel.querySelector("#leaveTemplateReset")?.addEventListener("click", () => { const searchInput = document.getElementById("leaveTemplateSearch"); if (searchInput) searchInput.value = ""; const select = document.getElementById("leaveTemplateStatus"); if (select) select.value = "all"; render(); });
  }
