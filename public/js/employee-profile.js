(() => {
  ...
  function populateLeaveForm() {
    ...
    setTextInput("leave-company", COMPANY.name);
    setTextInput("leave-company-email", COMPANY.email);
    setTextInput("leave-company-phone", COMPANY.phone);
    setTextInput("leave-company-address", COMPANY.address);
    setTextInput("leave-approver", COMPANY.approver);
  }
  ...
  function downloadLeaveForm(event) {
    ...
    const html = `<!doctype html><html><head><meta charset="UTF-8"><title>${COMPANY.name} Leave Request</title><style>body{font-family:Arial,sans-serif;margin:45px;color:#172033}.header{text-align:center;border-bottom:3px solid #00674f;padding-bottom:18px;margin-bottom:28px}.header h1{color:#00674f;margin-bottom:6px}.company{font-size:13px;line-height:1.6;color:#555}.row{display:flex;border-bottom:1px solid #ddd;padding:12px 0}.label{font-weight:bold;width:190px}.reason{min-height:120px;border:1px solid #ddd;padding:15px;margin-top:20px}.sign{display:flex;justify-content:space-between;margin-top:80px}.footer{margin-top:40px;text-align:center;font-size:12px;color:#777}</style></head><body><div class="header"><h1>${escapeHtml(COMPANY.name)}</h1><h2>EMPLOYEE LEAVE REQUEST FORM</h2><div class="company">${escapeHtml(COMPANY.address)}<br>${escapeHtml(COMPANY.phone)} • ${escapeHtml(COMPANY.email)}</div></div><div class="row"><span class="label">Employee Name</span><span>${escapeHtml(data.get("employeeName"))}</span></div><div class="row"><span class="label">Employee ID</span><span>${escapeHtml(data.get("employeeId"))}</span></div><div class="row"><span class="label">Department</span><span>${escapeHtml(data.get("department"))}</span></div><div class="row"><span class="label">Leave Type</span><span>${escapeHtml(data.get("leaveType"))}</span></div><div class="row"><span class="label">Start Date</span><span>${escapeHtml(data.get("startDate"))}</span></div><div class="row"><span class="label">End Date</span><span>${escapeHtml(data.get("endDate"))}</span></div><div class="row"><span class="label">Number of Days</span><span>${escapeHtml(data.get("days"))}</span></div><div class="row"><span class="label">Approver</span><span>${escapeHtml(data.get("approver"))}</span></div><div class="reason"><strong>Reason for Leave</strong><p>${escapeHtml(data.get("reason"))}</p></div><div class="sign"><div>Employee Signature: ____________________</div><div>Date: ____________________</div></div><div class="sign"><div>HR / Manager Signature: ____________________</div><div>Date: ____________________</div></div><div class="footer">${escapeHtml(COMPANY.hr)} • ${escapeHtml(COMPANY.email)} • ${escapeHtml(COMPANY.phone)}</div></body></html>`;
    const blob = new Blob([html], { type: "text/html;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a"); link.href = url; link.download = `${COMPANY.name.replace(/\s+/g, "-")}-Leave-Request-${currentEmployee.employeeId || "employee"}.html"; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url);
    closeModal("emp-leave-modal");
  }

  function calculateLeaveDays() { const from = byId("emp-leave-from")?.value, to = byId("emp-leave-to")?.value, days = byId("emp-leave-days"); if (!days || !from || !to) return; const difference = Math.round((new Date(`${to}T00:00:00`) - new Date(`${from}T00:00:00`)) / 86400000) + 1; days.value = difference > 0 ? difference : 0; }
  ...
  function setupButtons() {
    ...
    byId("emp-message-form")?.addEventListener("submit", e => { e.preventDefault(); alert(`Your message has been submitted to ${COMPANY.hr}.`); e.target.reset(); closeModal("emp-message-modal"); });
