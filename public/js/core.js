...
function applyAvatar(imagePath) {
  ...
  document.querySelectorAll(".settings_avatar-option").forEach((avatarOption) => {
    avatarOption.classList.toggle("active", avatarOption.getAttribute("src") === avatarPath);
  });
}
...
function syncProfileInfo() {
  ...
  const savedAvatar = localStorage.getItem("selectedAvatar") || user?.avatar || DEFAULT_AVATAR;
...
if (protectedPages.includes(currentPage) && (!hasSession || !hasToken)) {
